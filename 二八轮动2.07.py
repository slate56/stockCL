# 克隆自聚宽文章：https://www.joinquant.com/post/3769
# 标题：二八轮动小市值优化版 v2.0.7 ——修正市值排序（结贴，JQ数据没有问题）
# 作者：zzeric
...
def do_handle_data(context, data):
    ...
    if gr_index2 <= g.index_growth_rate and gr_index8 <= g.index_growth_rate:
        ...
        if g.day_count % g.period == 0:
            log.info("==> 满足条件进行调仓")
            buy_stocks = pick_stocks_ex(context, data)
...
#先用昨日收盘价计算一次市值，缩小分钟取数据范围，提高计算速度
def getStockPool():
    priceDict = history(1, unit="1d", field='close', security_list=g.total_codes, df=False, skip_paused=True, fq='pre')
    dataList = [(k,v[0]) for k,v in priceDict.items()]
    priceDf = pd.DataFrame(columns=["code","close"],data=dataList)
    mergeDf = pd.merge(priceDf,g.total_shares,on='code')
    mergeDf["market_cap"] = mergeDf["close"] * mergeDf["capitalization"]
    sortedDf = mergeDf.sort(['market_cap'], ascending=[1])
    return list(sortedDf.head(g.pick_stock_count)["code"])

# 选股
# 选取指定数目的小市值股票，再进行过滤，最终挑选指定可买数目的股票
def pick_stocks_ex(context,data):
    priceDict = history(1, unit="1m", field='close', security_list=g.stocks_pool, df=False, skip_paused=True, fq='pre')
    dataList = [(k,v[0]) for k,v in priceDict.items()]
    priceDf = pd.DataFrame(columns=["code","close"],data=dataList)
    mergeDf = pd.merge(priceDf,g.total_shares,on='code')
    mergeDf["market_cap"] = mergeDf["close"] * mergeDf["capitalization"]
    sortedDf = mergeDf.sort(['market_cap'], ascending=[1])
    codes = list(sortedDf["code"])
    ...
    if g.pick_by_pe:
        q = q.filter(
            ...
            #,valuation.code.in_(codes) #这里不用in_(codes)的方式是因为感觉慢很多
            )
    ...
    tmp_list = list(df['code'])
    stock_list = [code for code in codes if code in tmp_list] #这里用这
