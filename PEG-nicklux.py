...
感谢小兵哥对PEG选股策略的优化，我是根据小兵哥的策略进行了一些修改：
1、原来策略由于疯牛的时候市场普遍估值偏高，PEG的选股策略失效，所以无法吃下牛市行情，
    所以当非熊市行情时，PEG选股条件可适当放宽：PEG范围：0.5-1（PS:这里的非熊市的
    定义是使用了28小市值择时里的28双指数20天涨幅来判断）
2、原来策略里关于增长平稳一项，是使用了了‘inc_std < last_inc’，小兵哥也注释了不太合理，
    而我也这样觉得，又没想到好的办法，所以直接删了这条件
3、交易算法没有像小兵哥那样使用ES，VAR这些交易策略，也没有使用风控策略
    （使用过28择时，发现效果不好，原因是PEG选股活性不高，一旦错过机会就很难启动），
    仅仅是简单的每股固定1/5买入，如果选股不足5只，则相当于变相减仓了，
    个人觉得PEG选股本身就自带了一定的仓位风控效果，尤其是15年那波股灾
4、5日调仓，虽然频率看上去很高，但是其实交易频率没想象中高，毕竟是按照每季报的数据来计算的，
    影响调仓的因素还是股价
'''
